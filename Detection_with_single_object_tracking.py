...
tracking_net.load_state_dict(torch.load(join(realpath(dirname(__file__)), 'SiamRPNVOT.model'), map_location=device))
tracking_net.eval().to(device)

# Opencv DNN for YOLOv4_tiny
detection_net = cv2.dnn.readNet("yolov4_tiny_custom_best.weights", "yolov4_tiny_custom_test.cfg")
...
locking_timestamps=[0,0] # timestamps of the locking and unlocking of the object
...
while True:
    ...
    if cnt%frame_per_detection == 0 or not detection_initiated or not object_detected:
        ...
        # Object Detection
        (class_ids, scores, bboxes) = detection_model.detect(im, confThreshold=0.7, nmsThreshold=.35)
        bboxes = list(bboxes)
        ...
        print("Detection FPS: {}".format(1/((toc_detection - tic_detection)/ cv2.getTickFrequency())))
        ...
        for class_id, score, bbox in zip(class_ids, scores, bboxes):
            ...
            cv2.putText(im, class_name+""+"%.2f"%score, (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 3, (200,0,50), 1)
           

            cx, cy = x+w/2, y+h/2
            target_pos, target_sz = np.array([cx, cy]), np.array([w, h])
            tracker = SiamRPN_init(im, target_pos, target_sz, tracking_net, device)
        ...
        isLocked = True
        # Draw bbox
        cv2.putText(im, "Tracking Score: {:.2f}".format(state['score']), (10, 30), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 255), 1)
    ...
    isLocked_server = isLocked and large_enough(rect[2]-rect[0],rect[3]-rect[1],im_x,im_y) and in_locking_rect(rect,im_x,im_y)
    #isLocked_server = isLocked
    
    # Display frame
    cv2.putText(im, "FPS: %.2f" % (1/((toc - tic)/ cv2.getTickFrequency())), (10, 70), cv2.FONT_HERSHEY_PLAIN, 2, (200,0,50), 2)
    cv2.putText(im, "Frame: {}".format(cnt), (900, 40), cv2.FONT_HERSHEY_PLAIN, 3, (200,0,50), 2)
    ...
    if isLocked:
        cv2.rectangle(im, (rect[0], rect[1]), (rect[2], rect[3]), (0, 0, 255), 3)
        cv2.putText(im, "Locked", (450, 40), cv2.FONT_HERSHEY_PLAIN, 3, (0,0,255), 3)
        print("area: ",(rect[2]-rect[0])*(rect[3]-rect[1]))
    if not isLocked:
        cv2.putText(im, "NOTLocked", (450, 40), cv2.FONT_HERSHEY_PLAIN, 3, (5,110,5), 1)
    #if isLocked_server:
    if isLocked_server:
        if locking_timestamps == [0,0]:
            ...
            cv2.putText(im, "Locking time: {:.2f}".format(duration_tic-locking_timestamps[0]), (10, 100), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 3)
        cv2.putText(im, "Locked_for_server", (450, 80), cv2.FONT_HERSHEY_PLAIN, 2, (0,0,255), 3)
        cv2.rectangle(im, (rect[0], rect[1]), (rect[2], rect[3]), (0, 0, 255), 3)
    ...
    if not isLocked_server:
        if not locking_timestamps == [0,0]:
            ...
            if locking_timestamps[1]-locking_timestamps[0] > 5:
                print("Locking finished at {}".format(duration_tic)+" with duraiton more than 5 seconds")
                print("Locking_duration: {}".format(locking_timestamps[1]-locking_timestamps[0]))            
            else:
                #this wont be published to the server, just for the user to see
                print("Locking finished at {}".format(duration_tic)+" with duraiton less than 5 seconds")
                print("Locking_duration: {}".format(locking_timestamps[1]-locking_timestamps[0]))
            locking_timestamps = [0,0]
    

    #exp
    cv2.putText(im,"states: {}".format(states),(10,180),cv2.FONT_HERSHEY_PLAIN,2,(0,255,0),1)


    #Locking rectangle
    cv2.rectangle(im,(im_x//4,im_y//10),(im_x*3//4,im_y*9//10), (130,0,75),3)
    cv2.putText(im, "Clock: {:.2f}".format(duration_tic-start_time), (10,140), cv2.FONT_HERSHEY_PLAIN, 2, (150,250,250), 2)
    #resize for display
