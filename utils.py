...
def fsm(input_state,states):
    ...
    if input_state:
        ...
        if states == [True, True, True, True, True]:
            input_state = True
    return input_state, states

def area(m):
    return m[2]*m[3]

def large_enough(w,h,x,y):
    return w>0.05*x or h>0.05*y

def in_locking_rect(rectangle, x, y):
    #cx = (rectangle[0] + rectangle[2])/2
    #cy = (rectangle[1] + rectangle[3])/2
    return x//4 < rectangle[0] and rectangle[2] < 3*x//4 and y//10 < rectangle[1] and rectangle[3] < 9*y//10
    #return x//4 < cx < 3*x//4 and y//10 < cy < 9*y//10
